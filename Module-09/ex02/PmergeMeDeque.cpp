...
std::string PmergeMeDeque::toString() const
{
    std::stringstream ss;
    std::string result;
...
std::deque< std::deque<int> > PmergeMeDeque::groupDeque(const std::deque<int> &other, unsigned int groupsize) {
    std::deque< std::deque<int> > temp;
    unsigned int counter = 0;
    std::deque<int>::const_iterator it = other.begin();
    while (it != other.end()) 
    {
        if (counter % groupsize == 0)
        {
            temp.push_back(std::deque<int>());
        }
...
void PmergeMeDeque::pairAndSortGroups(std::deque< std::deque<int> > &temp, unsigned int groupsize) {
    std::stringstream debugss;
    for (std::deque< std::deque<int> >::iterator its = temp.begin(); its != temp.end(); its += 2) {
...
void PmergeMeDeque::insertFromPendChain(std::deque<SubGroup>& mainchain, std::deque<SubGroup>& pendchain) {
    ...
    while (1) {
        ...
        for (unsigned int i = currJacob; i > lowerbound; --i) {
            ...
            std::deque<SubGroup>::iterator upperit = std::find(mainchain.begin(), mainchain.end(), SubGroup(tagm));

            std::deque<SubGroup>::iterator itinsert = std::lower_bound(mainchain.begin(), upperit, pendchain[i - 2]);
...
void PmergeMeDeque::fordJohnsonSort(unsigned int iteration)
{
    ...
    temp.clear();
    temp = groupDeque(copy, groupsize);

    // 2.1 Labelling
    std::deque<SubGroup> mainchain;
    std::deque<SubGroup> pendchain;
    ...
    debugss << "\nPend chain: \n";
    for (std::deque<SubGroup>::iterator it = pendchain.begin(); it != pendchain.end(); ++it) {
        debugss << it->toStringsg() << "\n";
    }
    ...
    if (nonpart.empty()) {
        ...
        debugss << "[ ";
        for (std::deque<int>::iterator it = nonpart.begin(); it != nonpart.end(); ++it) {
            debugss << *it << " ";
        }
        debugss << "]";
    }
    ...
    for (std::deque<SubGroup>::iterator it = mainchain.begin(); it != mainchain.end(); ++it) {
        debugss << highlightlastnum(it->subdeq) << " ";
    }
    DEBUG_PRINT(debugss.str());
    debugss.str("");
...
unsigned int PmergeMeDeque::getJacobNo(unsigned int i)
{
    if (i == 0)
        return 1;
    if (i == 1)
...
PmergeMeDeque::SubGroup::SubGroup() {}

PmergeMeDeque::SubGroup::SubGroup(const SubGroup &other): tag(other.tag), lastnum(other.lastnum), subdeq(other.subdeq) {}

PmergeMeDeque::SubGroup &PmergeMeDeque::SubGroup::operator=(const SubGroup &other)
{
    if (this != &other) {
        this->tag = other.tag;
        this->lastnum = other.lastnum;
        this->subdeq = other.subdeq;
    }
    return *this;
}

PmergeMeDeque::SubGroup::SubGroup(std::string tag, const std::deque<int> &v)
    : tag(tag), subdeq(v)
{
    lastnum = 0;
    if (!v.empty()) {
...
PmergeMeDeque::SubGroup::SubGroup(std::string tag)
: tag(tag) {}
...
bool PmergeMeDeque::SubGroup::operator==(const SubGroup &other) const
{
...
bool PmergeMeDeque::SubGroup::operator<(const SubGroup &other) const
{
    ++PmergeMeDeque::comparisons;
    return (this->lastnum < other.lastnum);
}

std::string PmergeMeDeque::SubGroup::toStringsg() const
{
    std::stringstream ss;
    ss << tag << " ->\t" << highlightlastnum(subdeq);
    return ss.str();
}
